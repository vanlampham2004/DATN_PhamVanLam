...
from tests.store.test_ui_store import capture_store_ui
...
def main():
    baseline_dir = "baseline/store"
    report_dir = "reports/store"

    os.makedirs(baseline_dir, exist_ok=True)
    os.makedirs(report_dir, exist_ok=True)

    logger.info("START STORE UI TEST PIPELINE")

    try:
        screenshots = capture_store_ui()
        ...
        cases = generate_test_cases(
            text,
            feature_name="Store Page UI",
            n=5
        )

        excel_path = f"{report_dir}/test_cases_store.xlsx"
        ...
        logger.info("DONE STORE UI TEST")
